export const BLOG_TITLE = "makenowjust-labs/blog";
export const BLOG_BASE_URL = "https://makenowjust-labs.github.io/blog";
export const BLOG_AUTHOR = 'Hiroya Fujinami (a.k.a. TSUYUSATO "MakeNowJust" Kitsune)';
export const BLOG_DESCRIPTION = "MakeNowJust Laboratory Tech Blog";
export const BLOG_REPO = "makenowjust-labs/blog";
export const BLOG_YEAR = "2021-2026";
export const BLOG_TWITTER = "make_now_just";
export const BLOG_GITHUB = "makenowjust";

export const BLOG_BASE_PATH = process.env.BLOG_BASE_PATH || "";

...
<Alert color="info">
  **補題**:
  discrimination tree $$\mathcal{T}$$ と $$d \in D$$、任意の文字列 $$w \in \Sigma^\ast$$ について、次の式が成り立つ。
...
となり、目的の式が得られます[^new-d]。

[^new-d]: ここで、もし $$w_i \cdot d \in D$$ であれば目的の不等式が等式になってしまうので、$$w_i \cdot d$$ が $$D$$ に含まれていない、新しい節のラベルの文字列になるということも分かります。
